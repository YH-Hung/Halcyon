...
- [ ] **Step 1: Verify the tree is clean and tests pass**
...
- [ ] **Step 2: Create the annotated tag (after user confirms)**
...
- [ ] **Step 3: Create the GitHub Release (after user confirms)**
...
- [ ] **Step 4: Update CHANGELOG.md's 1.0.0 note** — the sentence "No GitHub Release or `v1.0.0` tag has been published yet" is now false; replace it with a link to the release. Commit:
...
- [ ] **Step 1: Write the failing test** — append to `tests/unit/test_error.cpp`:
...
- [ ] **Step 3: Implement** — in `include/halcyon/error.hpp`, extend the enum (insert before `Unknown`):
...
- [ ] **Step 1: Write the failing tests** — create `tests/unit/test_logging.cpp`:
...
- [ ] **Step 3: Implement the header** — create `include/halcyon/observability/logging.hpp`:
...
- [ ] **Step 1: Write the failing tests** — append to `tests/unit/test_pool.cpp` (it already includes `mock_cli_driver.hpp`; add `#include "capturing_logger.hpp"` and `#include <memory>` if missing):
...
- [ ] **Step 1: Write the failing test** — append to `tests/unit/test_cli_seam.cpp`:
...
- [ ] **Step 4: Verify** — unit suite green, and the stress target still compiles:
...
- [ ] **Step 1: Write the failing test** — append to `tests/unit/test_pool.cpp`:
...
- [ ] **Step 2: Build and verify it fails** — compile error: `PoolConfig` has no member `isolation`.
...
- [ ] **Step 2: Build and verify the tests fail** — compile error: no `Connection::begin(Isolation)`.
...
- [ ] **Step 1: Write the failing tests** — create `tests/unit/test_savepoint.cpp`:
...
- [ ] **Step 2: Register and verify failure** — add `unit/test_savepoint.cpp` to `tests/CMakeLists.txt`, build:
...
- [ ] **Step 3: Implement** — in `include/halcyon/transaction.hpp` (includes: add `<cctype>`, `<optional>`, `<string_view>`, `"halcyon/observability/logging.hpp"` as needed):
...
- [ ] **Step 1: Write the failing tests** — append to `tests/unit/test_savepoint.cpp`:
...
- [ ] **Step 2: Build and verify failure** — compile error: no `Transaction::nested`.

- [ ] **Step 3: Implement** — declare inside `Transaction` (public):
...
- [ ] **Step 1: Start Db2 and build the integration target**
...
- [ ] **Step 2: Write the tests** — create `tests/integration/test_db2_savepoints_isolation.cpp` (fixture mirrors `test_db2_roundtrip.cpp`: skip when `HALCYON_TEST_DSN` unset, own table):
...
- [ ] **Step 3: Run and verify against live Db2**
...
- [ ] **Step 4: Also re-run the unit suite, then tear down**
...
- [ ] **Step 1: Write the failing mock-contract tests** — append to `tests/unit/test_cli_seam.cpp`:
...
- [ ] **Step 2: Build and verify failure** — compile error: `ICliDriver` has no `fetchNext`.

- [ ] **Step 3: Implement the seam header** — `include/halcyon/detail/cli/driver.hpp`: add `<cstdint>` (present), `<functional>`, `<optional>` includes; add `GetDataChunk`, `ParamStreamSource`, and the four pure virtuals exactly as in the Interfaces block above (place them after `closeCursor`, before the transaction-control section, under a `// --- Streaming data path (v1.1) ---` banner).

- [ ] **Step 4: Implement the mock** — in `tests/unit/mock_cli_driver.hpp`:
...
- [ ] **Step 5: Stub the other two drivers**
...
- [ ] **Step 1: Write the failing tests** — create `tests/unit/test_lob.cpp`:
...
- [ ] **Step 2: Register and verify failure** — add `unit/test_lob.cpp` to `tests/CMakeLists.txt`; build → `halcyon/lob.hpp` not found.

- [ ] **Step 3: Implement** — create `include/halcyon/lob.hpp`:
...
- [ ] **Step 1: Write the failing tests** — create `tests/unit/test_streaming.cpp`:
...
- [ ] **Step 2: Register and verify failure** — add `unit/test_streaming.cpp` to `tests/CMakeLists.txt`; build → no `queryStreaming`.

- [ ] **Step 3: Declare in `connection.hpp` / `transaction.hpp`**
...
- [ ] **Step 4: Implement `include/halcyon/streaming.hpp`**
...
- [ ] **Step 1: Write the failing tests** — append to `tests/unit/test_streaming.cpp` (add `#include <fstream>` and `#include <sstream>`):
...
- [ ] **Step 2: Build and verify failure** — compile/link error: no `toString` on `LobReader`.

- [ ] **Step 3: Implement** — add the declarations to `class LobReader` (if not already present from Task 13) and these definitions after `LobReader::read` in `streaming.hpp`:
...
- [ ] **Step 1: Write the failing tests** — create `tests/unit/test_lob_write.cpp`:
...
- [ ] **Step 2: Register and verify failure** — add `unit/test_lob_write.cpp` to `tests/CMakeLists.txt`; build → no matching `execute` overload for a `LobSource` argument.
...
- [ ] **Step 1: Write the failing tests** — append to `tests/unit/test_streaming.cpp`:
...
- [ ] **Step 2: Build and verify failure** — no `Database::queryStreaming`.

- [ ] **Step 3: Implement** — in `include/halcyon/database.hpp` (which now includes `halcyon/streaming.hpp` via Task 13's umbrella? No — add `#include "halcyon/streaming.hpp"` explicitly to `database.hpp`), after `class QueryResult`:
...
- [ ] **Step 1: Refactor — extract `read_cell`**
...
- [ ] **Step 1: Implement** — file-local constant in the anonymous namespace:
...
- [ ] **Step 1: Start Db2 (as in Task 10) and write the tests** — create `tests/integration/test_db2_lobs.cpp`:
...
- [ ] **Step 3: Full unit suite, tear down, commit**
...
- [ ] **Step 1: Read the existing harness** — skim `tests/stress/correctness/test_stress_concurrency.cpp` and `support/concurrent_fake_driver.hpp`; mirror their setup (how a `Database` is opened over the fake, thread counts, iteration style).

- [ ] **Step 2: Write the scenario** — create `tests/stress/correctness/test_stress_savepoints.cpp` following that pattern:
...
- [ ] **Step 3: Run under both sanitizers**
...
- [ ] **Step 1: Extend `docs/guide/transactions.md`** with two sections (adapt heading levels to the page):
...
- [ ] **Step 2: Create `docs/guide/lobs.md`**:
...
- [ ] **Step 3: Extend `docs/guide/observability.md`**:
...
- [ ] **Step 4: `mkdocs.yml` nav** — after `Batch Operations`:

```yaml
      - Large Objects (LOBs): guide/lobs.md
```

- [ ] **Step 5: README feature bullets** — extend the feature list with savepoints/nested transactions, isolation levels, LOB streaming, and structured logging (one bullet each, matching the README's existing bullet style).

- [ ] **Step 6: Build the docs site with the strict link gate and commit**
...
- [ ] **Step 1: CHANGELOG** — move the v1.1 features from `Unreleased` into a new section:
...
- [ ] **Step 2: Version bump** — top-level `CMakeLists.txt`: `project(Halcyon VERSION 1.1.0 LANGUAGES CXX)`. Build and run `test_version` (`ctest --test-dir build -R version --output-on-failure`) — the expected version is injected from CMake, so it must pass without editing the test.

- [ ] **Step 3: Full verification (all four gates, per AGENTS.md)**
...
- [ ] **Step 5: Tag and release** — ⚠️ **User gate** (same as Task 0): after explicit confirmation,
